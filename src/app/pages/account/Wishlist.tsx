            ...
            <div className="pt-8 pb-16 px-4">
                ...
                {loading ? (
                    ...
                    <div className="bg-white rounded-3xl p-16 shadow-sm border border-dashed border-gray-200 text-center">
                        ...
                        <Link
                            to="/store"
                            className="inline-block px-8 py-3 bg-koala-navy text-white rounded-2xl hover:bg-koala-navy-hover transition-all font-bold text-sm"
                        >
                            스토어 구경하기
                        </Link>
                    </div>
                            ...
                            {wishlist.map((item: any) => (
                                <div key={item.skuCode} className="group bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100">
                                        ...
                                        <div className="relative aspect-square overflow-hidden bg-gray-50">
                                            ...
                                            {item.isLimitedEdition && (
                                                <div className="absolute top-2 left-2">
                                                    <span className="px-2 py-0.5 bg-koala-navy text-white text-[10px] font-bold rounded-lg">Limited</span>
                                                </div>
                                            )}
                                        </div>
                                    </Link>
                                    <div className="p-3">
                                        ...
                                        <p className="text-sm font-black mb-3">₩{(item.effectivePrice ?? 0).toLocaleString()}</p>
                                        <div className="flex gap-2">
                                            <button
                                                onClick={() => handleAddToCart(item.skuCode)}
                                                className="flex-1 flex items-center justify-center gap-1 py-2 bg-koala-navy text-white rounded-xl text-xs font-bold"
                                            >
